...
xmakeglyphfontspecs(XftGlyphFontSpec *specs, const Glyph *glyphs, int len, int x, int y)
{
	...
	for (code_idx = 0; code_idx < shaped.count; code_idx++) {
		...
		/* Skip image placeholder cells; images drawn separately. */
		if (glyphs[idx].mode & ATTR_IMAGE)
			continue;
...
xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y, int charlen)
{
	...
	/* Render strikethrough. Always use the fg color. */
...
xdrawline(Line line, int x1, int y1, int x2)
{
	...
	if (i > 0) {
		...
		if (base.mode & ATTR_IMAGE)
			xdrawimages(base, line, ox, y1, x2);
	}
}
