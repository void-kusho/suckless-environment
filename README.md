...
| The battery says `+` (charging) and the percentage still drops | The `+` is the kernel's `status`, which the EC sets whenever a charger is present — it describes the mode, not the balance. On the reference machine the battery is at 65 % health and tapers from ~84 %; under load the net goes negative. A charger with headroom (the original 65 W, plugged directly), a full calibration cycle, or a new battery. `/sys/class/power_supply/BAT1/charge_control_end_threshold` is Samsung's charge cap, kept in firmware. |
