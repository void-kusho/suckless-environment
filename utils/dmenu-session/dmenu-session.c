...
action_lock(void)
{
	...
	/* Valid betterlockscreen invocation: -l takes the EFFECT as its
	 * argument ("blur", "dim", ...), which selects a cached image by
	 * name (CUR_L_<effect>.png). Anything else -- like a stray option
	 * -- falls through to lockselect()'s default case and silently
	 * locks with the wrong/raw image. The cache lives in
	 * ~/.cache/betterlockscreen and is regenerated by the session's
	 * autostart.sh when missing. --display 1 places the loginbox on
	 * the DP-1 (primary) monitor. */
	const char *lock_cmd[] = {
